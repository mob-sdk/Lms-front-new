...
class UserTagAddView(UserTagMixin, CreateView):
    ...
    def get_form_kwargs(self):
        ...
        if 'instance' not in kwargs or not kwargs['instance']:
            kwargs.update(
                {'instance': self.form_class.get_base_object(self.instance)})
        return kwargs
    ...
    def configure(self, request):
        try:
            ...
            messages.error(request, _(
                "Server returned error '{error!s}'.").format(error=e))
...
class SignInAsUser(BaseRedirectMixin, BaseTemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages.error(request, _('Username "{username}" does not exist.').format(
