...
class LearningObject(UrlMixin, ModelWithInheritance):
    ...
    audience = models.IntegerField(choices=AUDIENCE.choices,
        default=AUDIENCE.COURSE_AUDIENCE, verbose_name=_('Audience'))
    category = models.ForeignKey(LearningObjectCategory, on_delete=models.CASCADE,
            related_name="learning_objects", verbose_name=_('Category'))
    course_module = models.ForeignKey(CourseModule, on_delete=models.CASCADE,
            related_name="learning_objects", verbose_name=_('Course module'))
    ...
    url = models.CharField(max_length=255,
        ...
        help_text=_("Input an URL identifier for this object."), verbose_name=_('Urlidentifier'))
    name = models.CharField(max_length=255, verbose_name=_('Name'))
    description = models.TextField(blank=True,
        help_text=_("Internal description is not presented on site."), verbose_name=_('Description'))
    use_wide_column = models.BooleanField(default=False,
        help_text=_("Remove the third info column for more space."), verbose_name=_('use wide column'))

    service_url = models.CharField(max_length=255, blank=True, verbose_name=_('Service url'))
    exercise_info = JSONField(blank=True)
    model_answers = models.TextField(blank=True,
        help_text=_("List model answer files as protected URL addresses."), verbose_name=_('Model answers'))
    templates = models.TextField(blank=True,
        help_text=_("List template files as protected URL addresses."), verbose_name=_('Templates'))
...
class CourseChapter(LearningObject):
    ...
    """
    generate_table_of_contents = models.BooleanField(default=False, verbose_name=_('Generate_table_of_contents'))
...
class BaseExercise(LearningObject):
    ...
    allow_assistant_viewing = models.BooleanField(default=True, verbose_name=_('Allow assistant viewing'))
    allow_assistant_grading = models.BooleanField(default=False, verbose_name=_('Allow assistant grading'))
    min_group_size = models.PositiveIntegerField(default=1, verbose_name=_('Min group size'))
    max_group_size = models.PositiveIntegerField(default=1, verbose_name=_('Max group size'))
    max_submissions = models.PositiveIntegerField(default=10, verbose_name=_('Max submissions'))
    max_points = models.PositiveIntegerField(default=100, verbose_name=_('Max points'))
    points_to_pass = models.PositiveIntegerField(default=40, verbose_name=_('Points to pass'))
    difficulty = models.CharField(max_length=32, blank=True, verbose_name=_('Difficulty'))
...
class LTIExercise(BaseExercise):
    ...
    """
    lti_service = models.ForeignKey(
        LTIService, on_delete=models.CASCADE, verbose_name='LTI服务')
    context_id = models.CharField(max_length=128, blank=True,
                                  help_text=_('Default: [hostname]/[course:url]/[instance:url]/'), verbose_name='内容id')
    resource_link_id = models.CharField(max_length=128, blank=True,
                                        help_text=_('Default: [aplusexercise:id]'), verbose_name='资源链接id')
    resource_link_title = models.CharField(max_length=128, blank=True,
                                           help_text=_('Default: the menu label of the LTI service'), verbose_name='资源链接名称')
...
class StaticExercise(BaseExercise):
    ...
    """
    exercise_page_content = models.TextField(verbose_name='练习页面内容')
    submission_page_content = models.TextField(verbose_name='提交页面内容')
...
class ExerciseWithAttachment(BaseExercise):
    ...
    files_to_submit = models.CharField(max_length=200, blank=True,
        help_text=_("File names that user should submit, use pipe character to separate files"), verbose_name='需要提交的文件')
    attachment = models.FileField(upload_to=build_upload_dir, verbose_name='附件')
