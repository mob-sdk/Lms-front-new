...
from lib.viewbase import BaseRedirectMixin, BaseView, BaseFormView
...
from comments.models import Comment, CommentMultip
...
class ExerciseView(BaseRedirectMixin, ExerciseBaseView, EnrollableViewMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        post = get_object_or_404(LearningObject, service_url=self.exercise.service_url)
    ...
    def post(self, request, *args, **kwargs):
        ...
        return self.render_to_response(self.get_context_data(
            page=page, students=students, submission=new_submission, post=post,comment_list=comment_list,))
...
class exerciseCommentsView(ExerciseBaseView, BaseFormView):
    access_mode = ACCESS.STUDENT
    template_name = "exercise/comments_form.html"
    form_class = exercise_comments
    post_url_name = "exercise-comments"
    ...
    def get_success_url(self):
        return self.exercise.get_exercise_url()
    ...
    def form_valid(self, form):
        ...
        con = Comment.objects.create(
            name = self.request.user.get_username(),
            text = comments,
            post = post
        )

        #Notification.send(self.profile, self.exercise)

        messages.success(self.request, _("评论已成功保存并且通知了提交者"))
        return super().form_valid(form)

class exerciseCommentsReplyView(ExerciseBaseView, BaseFormView):
    access_mode = ACCESS.STUDENT
    parent_id_kw = "parent_id"
    template_name = "exercise/comments_form.html"
    form_class = exercise_comments
    post_url_name = "exercise-comments-reply"

    def get_resource_objects(self):
        super().get_resource_objects()
        self.comment = get_object_or_404(
            Comment,
            pk = self._get_kwarg(self.parent_id_kw)
        )
        self.note('comment')
    ...
    def form_valid(self, form):
        ...
        if parent_id:
            ...
            con = CommentMultip.objects.create(
            name = self.request.user.get_username(),
            text = comments,
            commentobject = self.comment,
            )
            comm = get_object_or_404(CommentMultip, pk=con.id)
            comm.parent = con
            comm.save()
