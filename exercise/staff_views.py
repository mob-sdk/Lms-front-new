...
class AssessSubmissionView(SubmissionMixin, BaseFormView):
    ...
    def get_form_kwargs(self):
        ...
        country_list = ('优秀-完全掌握', '良好-大部分掌握', '一般-基本掌握', '错误-没有掌握')
        kwargs["data_list"] = country_list
        return kwargs
...
class CreateSubmissionView(ExerciseMixin, BaseRedirectView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(request, _("新的提交已经保存."))
...
class EditSubmittersView(SubmissionMixin, BaseFormView):
    ...
    def form_valid(self, form):
        form.save()
        messages.success(self.request, _("改变已经保存成功."))
        return super().form_valid(form)

    def form_invalid(self, form):
        messages.error(self.request, _("改变保存失败."))
